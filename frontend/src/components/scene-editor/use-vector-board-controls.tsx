...
import type { AuxweaveDocument, SceneObject } from '../../lib/auxweave-scene'
import {
...
import {
  type AuxweaveVectorBoardMeta,
  loadVectorBoardDocs,
  loadVectorBoards,
  mergeVectorBoardDocsForMeta,
  saveVectorBoardDocs,
  saveVectorBoards,
} from '../../lib/auxweave-vector-boards-storage'
...
export function useVectorBoardControls({
  ...
  const [boards, setBoards] = useState<AuxweaveVectorBoardMeta[]>([])
  ...
  const createVectorBoard = useCallback(() => {
    ...
    const board: AuxweaveVectorBoardMeta = {
      id,
