...
import { iconSvgNodeAttrs, sceneIconPaintValue } from '../../lib/auxweave-icon'
import type { SceneArrow, SceneObject, SceneText } from '../../lib/auxweave-scene'
import {
  blurPxFromPct,
  layoutSceneText,
...
import type { VectorBoardDocument } from '../../lib/auxweave-vector-board-document'
...
function objectFilterCss(obj: SceneObject) {
  ...
  if (obj.shadow) {
    const s = obj.shadow as Record<string, unknown>
    const offsetX = typeof s.offsetX === 'number' ? s.offsetX : typeof s.x === 'number' ? s.x : 0
    const offsetY = typeof s.offsetY === 'number' ? s.offsetY : typeof s.y === 'number' ? s.y : 4
    const shadowBlur = typeof s.blur === 'number' ? s.blur : 12

    if (typeof s.colorHex === 'string') {
      const alpha =
        Math.max(0, Math.min(100, typeof s.opacityPct === 'number' ? s.opacityPct : 100)) / 100
      const hex = s.colorHex.replace('#', '')
      ...
      filters.push(
        `drop-shadow(${offsetX}px ${offsetY}px ${shadowBlur}px rgba(${r},${g},${b},${alpha}))`,
      )
    } else if (typeof s.color === 'string') {
      filters.push(`drop-shadow(${offsetX}px ${offsetY}px ${shadowBlur}px ${s.color})`)
    }
  }
...
function elementDataAttrs(obj: SceneObject) {
  return {
    'data-element-id': obj.id,
    'data-element-type': obj.type,
    'data-element-x': obj.x,
    'data-element-y': obj.y,
    'data-element-width': obj.width,
    'data-element-height': obj.height,
    'data-element-rotation': obj.rotation,
  }
}
  ...
  if (obj.type === 'group') {
    return (
      <div
        style={{
          ...style,
          isolation: obj.isolateGroup ? 'isolate' : 'auto',
        }}
        data-Auxweave-scene-object
        {...elementDataAttrs(obj)}
