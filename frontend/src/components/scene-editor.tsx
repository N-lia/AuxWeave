...
import { cloneIconSvg } from '../lib/auxweave-icon'
import {
  Auxweave_ICON_DRAG_MIME,
  type AuxweaveIconDragPayload,
  parseIconDragPayload,
} from '../lib/auxweave-icon-drag'
import { loadImageMetadata } from '../lib/auxweave-image-proxy'
import { generateMoodboardCanvasPage } from '../lib/auxweave-moodboard-layout'
import {
...
import {
  layoutSceneText,
  renderAuxweaveDocumentToCanvas,
  renderAuxweaveDocumentToDataUrl,
  sceneTextLineHeight,
} from '../lib/auxweave-scene-render'
import { averageShadowUi, DEFAULT_SHADOW_UI, type ShadowUi } from '../lib/auxweave-shadow'
import {
  Auxweave_VECTOR_BOARD_DRAG_MIME,
  type VectorBoardDocument,
} from '../lib/auxweave-vector-board-document'
...
import {
  ensureWebMCPToolsRegistered,
  mountWebMCPEditorBridge,
} from '../lib/webmcp/webmcp-editor-bridge'
import {
  angleFromPoints,
  boundsIntersect,
  clampDimension,
...
import { CanvasRulers, type UserGuide } from './scene-editor/canvas-rulers'
...
function isPointerOnSceneObject(target: EventTarget | null) {
  return target instanceof Element && !!target.closest('[data-Auxweave-scene-object]')
}
...
function safeExportFileBaseName(name: string) {
  const cleaned = name
    ...
    .replace(/[^a-z0-9-_]+/gi, '-')
    .replace(/^-+|-+$/g, '')
  return cleaned || 'Auxweave'
}

function pageExportDocument(doc: AuxweaveDocument, page: AuxweavePage): AuxweaveDocument {
  return {
    ...doc,
...
async function renderExportPagePreviewDataUrl(
  ...
  try {
    await renderAuxweaveDocumentToCanvas(ctx, pageDoc, vectorBoardDocs, { transparent: false })
    return canvas.toDataURL('image/png')
  } catch (error) {
    console.error('[Auxweave] export page preview failed', error)
    return null
  }
}
...
function renumberPages(pages: AuxweavePage[]): AuxweavePage[] {
...
const SceneEditor = forwardRef<SceneEditorHandle, SceneEditorProps>(function SceneEditor(
  ...
  const historyRef = useRef<AuxweaveDocument[]>([])
  ...
  const [activeSnapGuideIds, setActiveSnapGuideIds] = useState<string[]>([])
  ...
  const rulerSelectedBounds = useMemo(() => {
    if (selectedSingle) {
      return {
        left: selectedSingle.x,
        top: selectedSingle.y,
        width: selectedSingle.width,
        height: selectedSingle.height,
      }
    }
    return selectionBounds
  }, [selectedSingle, selectionBounds])
  ...
  const elementToolbarLayout = useMemo(() => {
    ...
    // For single selection, use unrotated center and anchors so the toolbar position
    // remains rock-solid stable during object rotation.
    const cx = selectedSingle
      ? selectedSingle.x + selectedSingle.width / 2
      : selectionBounds.left + selectionBounds.width / 2
    const cy = selectedSingle
      ? selectedSingle.y + selectedSingle.height / 2
      : selectionBounds.top + selectionBounds.height / 2
    const halfH = selectedSingle ? selectedSingle.height / 2 : selectionBounds.height / 2

    const screenCenterX = cx * scale
    const screenTopAnchor = (cy - halfH) * scale
    const screenBottomAnchor = (cy + halfH) * scale

    // Toolbar dimensions and safety margin in screen space
    const approxToolbarWidth = 320
    const approxToolbarHeight = 44
    const minTopEdgeGap = 56

    // Compute rotation handle location in screen space if a single unlocked object is selected
    let handleScreenX: number | null = null
    let handleScreenY: number | null = null
    if (selectedSingle && !selectedSingle.locked) {
      const rotateCenterOffset = 34 / Math.max(scale, 0.01)
      const handleDy = selectedSingle.height / 2 + rotateCenterOffset
      const rad = (selectedSingle.rotation * Math.PI) / 180
      handleScreenX = (cx - handleDy * Math.sin(rad)) * scale
      handleScreenY = (cy + handleDy * Math.cos(rad)) * scale
    }

    type ToolbarCandidate = {
    ...
    const candidates: ToolbarCandidate[] = []
    const placements: Array<'above' | 'below'> = ['above', 'below']
    const dxOffsets = [0, 110, -110, 180, -180]

    for (const placement of placements) {
      const anchorY = placement === 'above' ? screenTopAnchor : screenBottomAnchor
      const toolbarCenterY =
        placement === 'above'
          ? anchorY - approxToolbarHeight / 2 - 10
          : anchorY + approxToolbarHeight / 2 + 10

      for (const dx of dxOffsets) {
        const candidateX = screenCenterX + dx
        let score = 0

        // 1. Distance & preference penalty
        if (placement === 'above') {
          score += 0
        } else {
          score += 15
        ...
        if (dx !== 0) {
          score += 25 + Math.abs(dx) * 0.1
        }

        // 2. Viewport edge penalty (too close to top of canvas)
        if (placement === 'above' && screenTopAnchor <= minTopEdgeGap) {
          score += 1000
        }

        // 3. Collision penalty with rotation handle
        if (handleScreenX !== null && handleScreenY !== null) {
          const distX = Math.abs(candidateX - handleScreenX)
          const distY = Math.abs(toolbarCenterY - handleScreenY)
          if (distX < approxToolbarWidth / 2 + 16 && distY < approxToolbarHeight / 2 + 16) {
            score += 800
          }
        }
  ...
  }, [selectionBounds, zoomPercent, scale, selectedSingle])
  ...
  const defaultShapeBox = useMemo(
    () => ({
      fontSize: Math.round(Math.max(48, Math.min(artboardW, artboardH) * 0.055)),
      shapeSize: Math.round(Math.min(artboardW, artboardH) * 0.28),
      lineW: Math.round(artboardW * 0.4),
      lineH: Math.round(artboardH * 0.1),
    }),
    [artboardW, artboardH],
  )
  ...
  const addText = useCallback(() => {
    addObjects([
      createCenteredObject({
        ...
        x: 0,
        y: 0,
        width: Math.round(artboardW * 0.55),
  ...
  const placeIconObject = useCallback(
    (
      payload: AuxweaveIconDragPayload,
      opts?: {
        x?: number
        y?: number
        origin?: 'center' | 'top-left'
  ...
  const copyElementToClipboard = useCallback(async () => {
    ...
    await navigator.clipboard.writeText(JSON.stringify({ AuxweaveClip: true, v: 2, objects }))
  }, [doc.objects, selectedIds])
  ...
  const pasteFromClipboard = useCallback(
    async (anchor?: { x: number; y: number }) => {
      ...
      if (!text) return
      let parsed: { AuxweaveClip?: boolean; objects?: unknown[] } | null = null
      try {
        parsed = JSON.parse(text) as { AuxweaveClip?: boolean; objects?: unknown[] }
      } catch {
        parsed = null
      }
      if (parsed?.AuxweaveClip && Array.isArray(parsed.objects)) {
        const objects = parsed.objects
          .map(
            row =>
              parseAuxweaveDocument({
                v: Auxweave_DOC_VERSION,
  ...
  const applyBlendModeToSelection = useCallback(
    (blendMode: SceneBlendMode) => {
      updateSelectedObjects(obj => ({ ...obj, blendMode }))
    },
    [updateSelectedObjects],
  )

  const applyFillOpacityToSelection = useCallback(
    (fillOpacityPct: number) => {
      updateSelectedObjects(obj => ({
        ...obj,
        fillOpacity: Math.max(0, Math.min(1, fillOpacityPct / 100)),
      }))
    },
    [updateSelectedObjects],
  )

  const applyIsolateGroupToSelection = useCallback(
    (isolateGroup: boolean) => {
      updateSelectedObjects(obj => ({ ...obj, isolateGroup }))
    },
    [updateSelectedObjects],
  )
  ...
  const downloadDocumentJson = useCallback((value: AuxweaveDocument) => {
    ...
    a.href = url
    a.download = 'auxweave-document.json'
    a.click()
  ...
  const loadDocument = useCallback(async (file: File) => {
    ...
    const next = parseAuxweaveDocument(parsed)
    if (!next) throw new Error('This file is not an Auxweave document.')
    applyingHistoryRef.current = true
    setDoc(next)
    setSelectedIds([])
    setTextEditingId(null)
    historyRef.current = [cloneAuxweaveDocument(next)]
    ...
    (opts?: ExportImageOptions) => {
      void (async () => {
        try {
          ...
          const fileBase = safeExportFileBaseName(persistDisplayNameRef.current || 'Auxweave')
          ...
          if (format === 'pdf') {
            ...
            for (const page of exportPages) {
              ...
              const url = await renderAuxweaveDocumentToDataUrl(pageDoc, vectorBoardDocs, {
                format: 'png',
                multiplier,
                transparent: false,
              })
          ...
          console.error('[Auxweave] export failed', error)
  ...
  // Mount all WebMCP editor bridge globals so agent tool-calls can drive the editor.
  // Tools are registered once via singleton (immune to Strict Mode double-invoke).
  useEffect(() => {
    ensureWebMCPToolsRegistered()
    if (typeof window !== 'undefined') {
      if (persistId) {
        ;(window as unknown as { __Auxweave_ACTIVE_DOC_ID__?: string }).__Auxweave_ACTIVE_DOC_ID__ =
          persistId
      }
    }
    const teardown = mountWebMCPEditorBridge({
      store: editorStore,
      getVectorBoardDocs: () => vectorBoardDocs,
      placeImageObject,
    })
    return teardown
  }, [editorStore, vectorBoardDocs, placeImageObject, persistId])
      ...
      const onMove = (e: PointerEvent) => {
        ...
        if (drag.kind === 'move') {
          ...
          if (drag.initialBounds) {
            ...
            setActiveSnapGuideIds(snap.activeUserGuideIds ?? [])
          } else {
            snapGuideXRef.current = null
            snapGuideYRef.current = null
            setSnapGuides([])
  ...
  const onWorkspacePointerDown = useCallback(
    (e: ReactPointerEvent<HTMLDivElement>) => {
      ...
      if (targetEl?.closest?.('[data-Auxweave-chrome]')) return
      if (textEditingId) {
        commitTextDraft()
      }
      setContextMenu(null)
  ...
  const onViewportContextMenu = useCallback(
    (e: ReactMouseEvent<HTMLDivElement>) => {
      e.preventDefault()
      const targetEl = e.target as HTMLElement | null
      const clickedObject = targetEl?.closest?.('[data-Auxweave-scene-object]')
      const clickedPage = targetEl?.closest?.('[data-Auxweave-page-id]')
      const clickedPageId = clickedPage?.getAttribute('data-Auxweave-page-id') ?? null
      const pt = pointerToScene(e.clientX, e.clientY)
      setContextMenu({
        x: e.clientX,
        y: e.clientY,
  ...
  const activatePage = useCallback(
    (pageId: string, options?: { selectBackground?: boolean }) => {
      commitTextDraft()
      setDoc(prev => (prev.activePageId === pageId ? prev : activateAuxweavePage(prev, pageId)))
  ...
  const addPage = useCallback(
    (afterPageId?: string) => {
      ...
      setDoc(prev => {
        ...
        const nextPage = createEmptyAuxweavePage(
        ...
        return activateAuxweavePage({ ...prev, pages: nextPages }, nextPage.id)
      })
  ...
  const duplicatePage = useCallback(
    (sourcePageId?: string) => {
      ...
      setDoc(prev => {
        ...
        const duplicatedPage = createAuxweavePage({
        ...
        return activateAuxweavePage({ ...prev, pages: nextPages }, duplicatedPage.id)
      })
  ...
  const syncMoodboardToCanvas = useCallback(
    (board: any) => {
      if (!board) return
      commitTextDraft()
      const layout = generateMoodboardCanvasPage(board)
      const pageId = `page-moodboard-${board.id}`
      const pageName = `🎨 Moodboard: ${board.name}`

      setDoc(prev => {
        const existingIdx = prev.pages.findIndex(p => p.id === pageId)
        const moodboardPage: AuxweavePage = {
          id: pageId,
          name: pageName,
          artboard: layout.artboard,
          bg: { type: 'solid', color: '#0F131C' },
          objects: layout.objects,
        }

        const pages = [...prev.pages]
        if (existingIdx >= 0) {
          pages[existingIdx] = moodboardPage
        } else {
          pages.push(moodboardPage)
        }

        return activateAuxweavePage({ ...prev, pages }, pageId)
      })

      setPendingPageScrollId(pageId)
      clearPageInteractionState()
    },
    [clearPageInteractionState, commitTextDraft, setDoc],
  )
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ;(
        window as unknown as { __Auxweave_SYNC_MOODBOARD_CANVAS__?: (board: any) => void }
      ).__Auxweave_SYNC_MOODBOARD_CANVAS__ = syncMoodboardToCanvas
    }
  }, [syncMoodboardToCanvas])
  ...
  useLayoutEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      const pageEl = viewport.querySelector<HTMLElement>(
        `[data-Auxweave-page-id="${pendingPageScrollId}"]`,
      )
      if (pageEl) {
  ...
  useEditorKeyboardShortcuts({
    ...
    ungroupSelection,
    onToggleRulersRequest: () => setShowRulers(s => !s),
  })
  ...
  const onViewportDragOver = useCallback((e: React.DragEvent<HTMLDivElement>) => {
    if (
      e.dataTransfer.types.includes(Auxweave_VECTOR_BOARD_DRAG_MIME) ||
      e.dataTransfer.types.includes(Auxweave_ICON_DRAG_MIME) ||
  ...
  const onViewportDrop = useCallback(
    (e: React.DragEvent<HTMLDivElement>) => {
      e.preventDefault()
      const pt = pointerToScene(e.clientX, e.clientY)
      const boardId = e.dataTransfer.getData(Auxweave_VECTOR_BOARD_DRAG_MIME)
      if (boardId) {
  ...
  const selectionEffectsFooterSlot = hasObjectSelected ? (
    <>
      ...
      <TransparencyToolbarPopover
        ...
        fillOpacityPct={
          selectedSingle?.fillOpacity !== undefined
            ? Math.round(selectedSingle.fillOpacity * 100)
            : 100
        }
        blendMode={selectedSingle?.blendMode || 'normal'}
        isolateGroup={selectedSingle?.isolateGroup || false}
        onChange={applyOpacityToSelection}
        onFillOpacityChange={applyFillOpacityToSelection}
        onBlendModeChange={applyBlendModeToSelection}
        onIsolateGroupChange={
          selectedSingle?.type === 'group' ? applyIsolateGroupToSelection : undefined
        }
      />
        ...
        <div ref={editorChromeRef} className="relative flex min-h-0 flex-1 flex-col">
          ...
          >
            <CanvasRulers
              ...
              artboardW={artboardW}
              artboardH={artboardH}
              selectedBounds={rulerSelectedBounds}
              guides={userGuides}
              onGuidesChange={setUserGuides}
              containerRef={viewportRef}
              artboardRef={artboardOuterRef}
          ...
          <EditorSidePanels
            ...
            persistId={persistId}
            onSyncMoodboardToCanvas={syncMoodboardToCanvas}
          />
