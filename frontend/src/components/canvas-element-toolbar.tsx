...
const VIEWPORT_CONTAIN_PAD = { top: 30, left: 28, right: 8, bottom: 8 }
...
function containmentDeltaForRect(
  rect: DOMRect,
  vp: DOMRect,
  pad: number | { top: number; right: number; bottom: number; left: number },
): { x: number; y: number } {
  const padObj = typeof pad === 'number' ? { top: pad, right: pad, bottom: pad, left: pad } : pad
  const innerL = vp.left + padObj.left
  const innerR = vp.right - padObj.right
  const innerT = vp.top + padObj.top
  const innerB = vp.bottom - padObj.bottom
