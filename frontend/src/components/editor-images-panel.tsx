...
function unsplashReferralLink(absoluteUrl: string): string {
  try {
    ...
    u.searchParams.set('utm_source', 'Auxweave')
...
export default function EditorImagesPanel({ open, onClose, placeImageObject }: Props) {
  ...
  const addPhoto = useCallback(
    async (photo: UnsplashPhoto) => {
      ...
      try {
        ...
        if (placeImageObject) {
          const r = await placeImageObject(photo.urls.regular, {
            origin: 'center',
            width,
            height,
          })
