...
} from './auxweave-document'
import type { VectorBoardDocument } from './auxweave-vector-board-document'
import {
  clearAuxweaveVectorBoardStorage,
  loadVectorBoardDocs,
  loadVectorBoards,
  saveVectorBoardDocs,
  saveVectorBoards,
} from './auxweave-vector-boards-storage'

const DB_NAME = 'auxweave-editor'
const DB_VERSION = 1
const STORE = 'documents'

export type AuxweaveEditorIdbRecord = {
  id: string
  updatedAt: number
  document: AuxweaveDocument
  storageKind: Exclude<AuxweaveDocumentStorageKind, 'invalid'>
  /** User-visible file name (optional on legacy rows). */
  name?: string
}

type StoredAuxweaveEditorIdbRecord = {
  id: string
  updatedAt: number
  document: unknown
  name?: string
...
function normalizeEditorRecord(
  row: StoredAuxweaveEditorIdbRecord | null | undefined,
): AuxweaveEditorIdbRecord | null {
  if (!row || typeof row.id !== 'string') return null
  const storageKind = getAuxweaveDocumentStorageKind(row.document)
  if (storageKind === 'invalid') return null
  const document = parseAuxweaveDocument(row.document)
...
export async function idbGetEditorRecord(id: string): Promise<AuxweaveEditorIdbRecord | null> {
  const db = await openDb()
  try {
    return await new Promise<AuxweaveEditorIdbRecord | null>((resolve, reject) => {
      const tx = db.transaction(STORE, 'readonly')
      tx.onerror = () => reject(tx.error ?? new Error('idb read failed'))
      const r = tx.objectStore(STORE).get(id)
      r.onerror = () => reject(r.error ?? new Error('idb get failed'))
      r.onsuccess = () => {
        resolve(normalizeEditorRecord((r.result as StoredAuxweaveEditorIdbRecord | undefined) ?? null))
      }
    })
  } finally {
    db.close()
...
export async function idbGetDocument(id: string): Promise<AuxweaveDocument | null> {
  const row = await idbGetEditorRecord(id)
  return row?.document ?? null
}

export type AuxweaveEditorIdbListItem = {
  id: string
  name: string
  updatedAt: number
...
export async function idbListDocuments(): Promise<AuxweaveEditorIdbListItem[]> {
  const db = await openDb()
  try {
    return await new Promise<AuxweaveEditorIdbListItem[]>((resolve, reject) => {
      const tx = db.transaction(STORE, 'readonly')
      tx.onerror = () => reject(tx.error ?? new Error('idb list failed'))
      const r = tx.objectStore(STORE).getAll()
      r.onerror = () => reject(r.error ?? new Error('idb getAll failed'))
      r.onsuccess = () => {
        const rows = (r.result as StoredAuxweaveEditorIdbRecord[])
          .map(row => normalizeEditorRecord(row))
          .filter((row): row is AuxweaveEditorIdbRecord => row != null)
        const items: AuxweaveEditorIdbListItem[] = rows.map(row => ({
          id: row.id,
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      tx.objectStore(STORE).put({
        ...
        document,
        name,
      } satisfies AuxweaveEditorIdbRecord)
    })
  } finally {
    db.close()
  }
...
export async function idbDeleteDocument(id: string): Promise<void> {
  ...
  clearAuxweaveVectorBoardStorage(id)
}
...
export async function idbDuplicateDocument(sourceId: string): Promise<string | null> {
  ...
  const docClone = cloneAuxweaveDocument(row.document)
