...
import type { AuxweaveDocument, SceneArrow, SceneLine, SceneObject, SceneText } from './auxweave-document'
import { iconSvgToDataUrl } from './auxweave-icon'
import { getExportSafeImageUrl } from './auxweave-image-proxy'
import { shadowColorString } from './auxweave-shadow'
import {
  flattenVisibleStrokes,
  type VectorBoardDocument,
  type VectorBoardStroke,
} from './auxweave-vector-board-document'
import { samplePenAnchorsToPolyline } from './auxweave-vector-pen-bezier'
...
export async function preloadFontsForDocument(doc: AuxweaveDocument): Promise<void> {
...
async function drawSceneObject(
  ...
  ctx.globalCompositeOperation = mapBlendModeToComposite(obj.blendMode)
  applyShadow(ctx, obj)
...
export async function renderAuxweaveDocumentToCanvas(
  ctx: CanvasRenderingContext2D,
...
export async function renderAuxweaveDocumentToDataUrl(
  doc: AuxweaveDocument,
  vectorBoardDocs: Record<string, VectorBoardDocument>,
  opts?: {
    format?: 'png' | 'jpg' | 'webp'
  ...
  await renderAuxweaveDocumentToCanvas(ctx, doc, vectorBoardDocs, {
