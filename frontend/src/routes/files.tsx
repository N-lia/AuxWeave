...
import { parseAuxweaveDocument } from '../lib/auxweave-document'
import { AuxweaveDocumentPreviewEvictPersistId } from '../lib/auxweave-document-preview'
import {
  type AuxweaveEditorIdbListItem,
  idbDeleteDocument,
  idbListDocuments,
  idbMigrateLegacyDocument,
  idbPutDocument,
} from '../lib/auxweave-editor-idb'
import { downloadAuxweaveJsonForId } from '../lib/auxweave-files-export'
...
function FilesPage() {
  const [items, setItems] = useState<AuxweaveEditorIdbListItem[] | null>(null)
  ...
  const bulkDownload = useCallback(() => {
    ...
    void (async () => {
      try {
        for (const id of ids) {
          await downloadAuxweaveJsonForId(id)
          await new Promise(r => setTimeout(r, 140))
        }
      } catch (err) {
        posthog.captureException(err)
        console.error('[Auxweave] bulk download failed', err)
      }
    })()
  }, [selectedIds, posthog])
    ...
    void (async () => {
      try {
        for (const id of ids) {
          ...
          AuxweaveDocumentPreviewEvictPersistId(id)
        }
        setSelectedIds(prev => prev.filter(id => !ids.includes(id)))
        refreshList()
      } catch (err) {
        posthog.captureException(err)
        console.error('[Auxweave] delete failed', err)
      }
    })()
    ...
    async (file: File) => {
      ...
      try {
        ...
        try {
          ...
          posthog.captureException(err)
          setImportError(
            'That file is not valid JSON. Choose an exported Auxweave JSON document and try again.',
          )
          return
        }
        const document = parseAuxweaveDocument(raw)
        if (!document) {
          setImportError(
            'This JSON file could not be imported. Try an Auxweave export or a legacy Fabric-based Auxweave file.',
          )
          return
        }
  ...
  const requestOpenFile = useCallback(
    (row: AuxweaveEditorIdbListItem, source: 'thumbnail' | 'title' | 'menu') => {
      if (row.isLegacy) {
        setMigrationDialog({
          ids: [row.id],
          title: 'Convert this file first',
          message: `"${row.name}" was made in an older version of Auxweave. Convert it to the new editor before opening it.`,
  ...
  const requestMigrateAll = useCallback(() => {
    ...
    setMigrationDialog({
      ...
      message:
        legacyItems.length === 1
          ? 'This file was saved in an older version of Auxweave. Convert it now so it opens normally in the new editor.'
          : 'These files were saved in an older version of Auxweave. Convert them now so they open normally in the new editor.',
